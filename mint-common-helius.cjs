...
// Usage: node mint-common-helius.cjs
...
async function mintCommonNFT() {
  const endpoint = `https://api.helius.xyz/v0/token-metadata/nft?api-key=${HELIUS_API_KEY}`;
  ...
  const nftData = {
    ...
    updateAuthority: RECEIVER_WALLET,
    payer: RECEIVER_WALLET
  };
